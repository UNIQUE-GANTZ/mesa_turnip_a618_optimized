...
ir3_nir_post_finalize(struct ir3_shader *shader)
{
   ...
   if (compiler->gen >= 6 && s->info.stage == MESA_SHADER_FRAGMENT &&
       ...
       * Converte operações float32 em float16 em todos os estágios de fragmento
       * utilizando nir_lower_mediump_outputs. No A618, as unidades ALU FP16
       * operam em throughput duplo comparado ao FP32, maximizando o desempenho
       * em ambiente de emulação (Winlator/Box64).
       * A máscara UINT64_MAX cobre todos os varyings de saída disponíveis.
       */
