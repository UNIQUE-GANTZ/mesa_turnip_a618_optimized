                     ...
                     struct tu_renderpass_result **autotune_result)
{
    ...
    * The autotune bypass (sysmem path) is permanently disabled here.
    * On A618 in Winlator/Box64 emulation, GMEM tiled rendering always
    * outperforms sysmem due to the limited memory bandwidth available.
    * use_sysmem is hardcoded to false; autotune result is still collected
    * for statistics but does not influence the rendering path decision.
    * Seção 4 do documento de diretrizes: Gestão de Residência GMEM-Centric.
    */
