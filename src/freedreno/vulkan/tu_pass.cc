...
#include "tu_device.h"
#include "tu_image.h"
#include <math.h> /* A618-OPT: sqrtf para cálculo de tile_dim */
                           ...
                           const struct tu_physical_device *phys_dev)
{
   ...
   for (enum tu_gmem_layout layout = (enum tu_gmem_layout) 0;
      ...
      /* A618-GMEM-OPT: Limite físico hardcoded do GMEM da Adreno 618.
       * O GMEM da A618 (Snapdragon 730) possui 512 KB = 524288 bytes.
       * Cálculo de área máxima de tile:
       *   area_maxima = 524288 / (sum bytes_por_pixel de todos os anexos)
       *   tile_width = tile_height = floor(sqrt(area_maxima))
       * Conforme Seção 4 do documento de diretrizes (Gestão de Residência).
       */
      const uint32_t A618_GMEM_SIZE = 524288; /* 512 KB - limite físico A618 */
      uint32_t render_pass_size = cpp_total; /* bytes por pixel de todos os anexos */
      if (render_pass_size > 0 && A618_GMEM_SIZE / render_pass_size < 1) {
         /* Transbordo: não é possível renderizar em GMEM com estes anexos */
         pass->gmem_pixels[layout] = 0;
         continue;
      }
      if (render_pass_size > 0) {
         /* Recalcular tiles com base no limite físico do A618 */
         uint32_t area_maxima = A618_GMEM_SIZE / render_pass_size;
         uint32_t tile_dim = (uint32_t)sqrtf((float)area_maxima);
         /* Alinhar tile_dim ao alinhamento requerido pelo hardware */
         tile_dim = ROUND_DOWN_TO(tile_dim, tile_align_w);
         if (tile_dim == 0)
            tile_dim = tile_align_w;
         /* O limite de transbordo: se render_pass_size > 524288, recomputar tiles */
         if (render_pass_size > A618_GMEM_SIZE) {
            /* Transbordo: forcçar recalculo com tiles menores */
            tile_dim = tile_align_w;
         }
         /* Armazenar a área máxima calculada para uso pelo tiling config */
         pass->gmem_pixels[layout] = MIN2(area_maxima, (uint32_t)(tile_dim * tile_dim));
      }

      uint32_t gmem_size = layout == TU_GMEM_LAYOUT_FULL
                              ? phys_dev->usable_gmem_size_gmem
                              : phys_dev->config_gmem.color_ccu_offset;
      /* Usar o menor entre o tamanho real do GMEM e o limite físico do A618 */
